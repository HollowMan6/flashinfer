...
class FusedMoeRunner : public tvm::ffi::ModuleObj {
  ...
  void runMoe(TensorView output, TensorView input, TensorView token_selected_experts,
    ...
    if (lora_info.use_lora) {
      ...
      lora_params = ::tensorrt_llm::kernels::LoraParams(
          ...
          lora_info.fc1_lora_impl, lora_info.fc2_lora_impl,
 ...
 private:
  ...
  static size_t loraMetadataWorkspaceSize(int64_t expanded_num_rows, int num_lora_modules) {
    ...
    return static_cast<size_t>(num_lora_modules) * (rank_size + ptr_size);
  }
  ...
  LoraInfo prepareLoraInfo(int64_t num_rows, int experts_per_token, int64_t hidden_size,
    ...
    checkRankTensor(fc1_lora_ranks.value(), num_lora_adapters, "fc1_lora_ranks");
    ...
    int64_t inferred_max_rank = lora_max_rank;
    if (is_gated) {
    ...
    TVM_FFI_ICHECK_GT(inferred_max_rank, 0)
    ...
    info.fc1_lora_impl = std::make_shared<::tensorrt_llm::kernels::LoraImpl>(
        ...
        static_cast<int>(inferred_max_rank), nullptr);
    ...
    size_t const metadata_workspace_size =
        loraMetadataWorkspaceSize(expanded_num_rows, num_lora_modules);
    size_t const device_run_workspace_size = std::max(
    ...
    info.use_lora = true;
    info.max_low_rank = inferred_max_rank;
